...
use crate::{mul_base_yielding, mul_scalar_yielding, CurveParams, Fe, Point};
...
pub async fn verify_yielding<const N: usize>(
    c: &CurveParams,
    pk: &[u8],
    msg_hash: &[u8],
    r: &[u8],
    ...
    let u1_g = mul_base_yielding(c, &u1_limbs, comb, comb_d, comb_t, budget).await;
    let u2_q = mul_scalar_yielding(c, &q, &u2_limbs, budget).await;
    let r_pt = u1_g.add(c, &u2_q);
...
pub async fn sign_yielding<const N: usize>(
    c: &CurveParams,
    sk: &[u8],
    msg_hash: &[u8],
    k_nonce: &[u8],
    ...
    let k_limbs = k.to_int(c);
    let r_pt = mul_base_yielding(c, &k_limbs, comb, comb_d, comb_t, budget).await;
    let (rx, _) = r_pt.to_affine(&c.field).ok_or(Error::BadPoint)?;
