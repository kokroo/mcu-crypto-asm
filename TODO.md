...
| **Ed25519** | SSH, Signal, Matter | ✅ **DONE**<br>(Point Ops / Scalarmul) | ✅ **DONE**<br>(9.3ms add, 2.3s scalarmult @ 12MHz) | [cortex25519](https://github.com/embassy-rs/cortex25519)<br>*(2.0×–2.8×)* | [lib25519](https://lib25519.cr.yp.to/)<br>*(2.2×–3.5×)* | [esp-x25519](https://github.com/aCinal/esp-x25519)<br>*(2.2×–3.2×)* | ✅ **Production** (T1, T2 Done) |
| **Poly1305** | WireGuard, TLS 1.3 | ✅ **DONE**<br>(~17 c/byte total) | ✅ **DONE**<br>(~127 c/byte @ 12MHz, 7.3k cyc) | [poly1305-donna](https://github.com/floodyberry/poly1305-donna)<br>*(2.5×–3.5×)* | [riscv-crypto](https://github.com/riscv/riscv-crypto)<br>*(3.0×–4.5×)* | [poly1305-donna](https://github.com/floodyberry/poly1305-donna)<br>*(3.0×–4.0×)* | ✅ **Production** (T1, T2 Done) |
| **ChaCha20** | WireGuard, TLS 1.3 | ✅ **DONE**<br>(~30 c/byte bare-metal) | ✅ **DONE**<br>(~62 c/byte block, 82 c/byte bulk) | [OpenSSL chacha](https://github.com/openssl/openssl/blob/master/crypto/chacha/asm/chacha-armv4.pl)<br>*(1.5×–2.0×)* | [riscv-crypto](https://github.com/riscv/riscv-crypto)<br>*(1.8×–2.5×)* | [chacha-opt](https://github.com/floodyberry/chacha-opt)<br>*(1.8×–2.5×)* | ✅ **Production** (T1, T2 Done) |
| **RSA-2048 / 4096** | Secure Boot, PKI, TLS | ✅ **DONE**<br>(345k cycles RSA-2048) | ✅ **DONE**<br>(2.38s verify @ 12MHz, 28.6M cyc) | [Emill/rsa-armv7](https://github.com/Emill/rsa-armv7)<br>*(2.0×–3.0×)* | [riscv-crypto](https://github.com/kostoffelen/riscv-crypto)<br>*(2.5×–3.5×)* | [BearSSL rsa_i31](https://bearssl.org/)<br>*(2.5×–3.5×)* | ✅ **Production** (T1, T2 Done) |
| **secp256k1** | Bitcoin, Wallets | ✅ **DONE**<br>(~11.7k dbl, 101ms scalarmult) | ✅ **DONE**<br>(~150k dbl, 6.8s scalarmult @ 12MHz) | [micro-ecc arm](https://github.com/kmackay/micro-ecc)<br>*(2.0×–2.8×)* | [libsecp256k1](https://github.com/bitcoin-core/secp256k1)<br>*(2.2×–3.2×)* | [libsecp256k1](https://github.com/bitcoin-core/secp256k1)<br>*(2.2×–3.0×)* | ✅ **Production** (T1, T2 Done) |
...
| **SHA-512 / SHA-384** | Ed25519, P-384, Hashes | ✅ **DONE**<br>(11.7kc/blk, ~97 cpb)| ✅ **DONE**<br>(~219 c/byte @ 12MHz, 26.3k c/blk) | [OpenSSL sha512](https://github.com/openssl/openssl/blob/master/crypto/sha/asm/sha512-armv4.pl)<br>*(1.8×–2.5×)* | [riscv-crypto](https://github.com/riscv/riscv-crypto)<br>*(2.0×–3.0×)* | [OpenSSL sha512](https://github.com/openssl/openssl/blob/master/crypto/sha/asm/sha512-armv4.pl)<br>*(2.0×–3.0×)* | ✅ **Production** (T1, T2 Done) |
| **Keccak / SHAKE** | ML-KEM, ML-DSA, Hashes | ✅ **DONE**<br>(15.6k cycles / 24-rnd) | ✅ **DONE**<br>(~30.7k cycles / 24-rnd @ 12MHz) | [XKCP armv7m](https://github.com/XKCP/XKCP)<br>*(1.8×–2.5×)* | [pqriscv keccak](https://github.com/mupq/pqriscv)<br>*(2.0×–3.5×)* | [XKCP generic](https://github.com/XKCP/XKCP)<br>*(2.0×–3.2×)* | ✅ **Production** (T1, T2 Done) |
...
| **GHASH (GCM)** | AES-GCM Authentication | ✅ **DONE**<br>(820c/blk, ~51 cpb) | ✅ **DONE**<br>(~202 c/byte @ 12MHz, 3.6k c/blk) | [OpenSSL ghash](https://github.com/openssl/openssl/blob/master/crypto/modes/asm/ghash-armv4.pl)<br>*(1.8×–2.5×)* | [riscv-crypto](https://github.com/riscv/riscv-crypto)<br>*(8× with [Zbkc](https://github.com/riscv/riscv-crypto))* | [BearSSL ghash](https://bearssl.org/)<br>*(2.0×–3.0×)* | ✅ **Production** (T1, T2 Done) |
...
- [x] ✅ **Poly1305 One-Time Authenticator (RFC 8439)**:
  ...
  - [x] ✅ Target 2 (ARMv6-M Cortex-M0/M0+) constant-time evaluation. Hardware verification on physical Target 2 (`nucleo-stm32c031c6` and `nucleo-stm32g071rb` @ 12 MHz) via Teleprobe (100% PASS on RFC 8439 test vector [610 µs, 7.3k cyc] and 1024-byte bulk throughput @ 127 c/byte).
- [x] ✅ **ChaCha20 Stream Cipher**:
  ...
  - [x] ✅ Target 2 (ARMv6-M Cortex-M0/M0+) constant-time 32-bit quarter-round implementation. Hardware verification on physical Target 2 (`nucleo-stm32c031c6` and `nucleo-stm32g071rb` @ 12 MHz) via Teleprobe (100% PASS on RFC 8439 Section 2.3.2 block function [335 µs, ~62 c/byte], Section 2.4.2 encryption [915 µs], and 1024-byte bulk throughput @ 82 c/byte).
...
- [x] ✅ **Target 2: ARMv6-M (Cortex-M0 / Cortex-M0+) GHASH Backend**: Bit-reversed 32-bit constant-time Karatsuba polynomial multiplier (BearSSL `ghash_ctmul32`) with zero precomputed tables. Hardware verified on physical Target 2 (`nucleo-stm32c031c6` and `nucleo-stm32g071rb` Cortex-M0+ @ 12 MHz) via Teleprobe (100% PASS on NIST SP 800-38D KATs, single-block gmult @ 305 µs [3.6k cyc], 2-block streaming @ 579 µs [217 cpb], 1024B bulk @ 202 c/byte).
- [x] ✅ **Target 2: ARMv6-M (Cortex-M0 / Cortex-M0+) SHA-512 / SHA-384 Backend**: BearSSL-optimized boolean logic minimization (`ch`, `maj`) for 64-bit word pairs without 64-bit library bloat. Hardware verified on physical Target 2 (`nucleo-stm32c031c6` and `nucleo-stm32g071rb` Cortex-M0+ @ 12 MHz) via Teleprobe (100% PASS on FIPS 180-4 SHA-512/384 KATs [2.19 ms/blk, 26.3k cyc], 1024B bulk @ 219 c/byte).
- [x] ✅ **Target 2: ARMv6-M (Cortex-M0 / Cortex-M0+) Keccak-p[1600] / SHA-3 / SHAKE Backend**: In-place 32-bit word permutation and FIPS 202 sponge engine. Hardware verified on physical Target 2 (`nucleo-stm32c031c6` and `nucleo-stm32g071rb` Cortex-M0+ @ 12 MHz) via Teleprobe (100% PASS on 24-round permutation [2,563 µs, 30.7k cyc], SHA3-256 [36.2k cyc], SHA3-512 [38.0k cyc], and SHAKE-128 [36.2k cyc] KATs).
- [x] ✅ **Target 2: ARMv6-M (Cortex-M0 / Cortex-M0+) secp256k1 Backend**: Pure 32-bit/64-bit Comba multiplication and cross-term doubling Comba squaring without 128-bit runtime emulation. Hardware verified on physical Target 2 (`nucleo-stm32c031c6` Cortex-M0+ @ 12 MHz) via Teleprobe (100% PASS on generator curve validation, point doubling [12.5 ms, 150.7k cyc], RFC 6979 public key derivation [6.84 s], and ECDH shared secret key agreement [6.84 s]).
- [x] ✅ **Target 2: ARMv6-M (Cortex-M0 / Cortex-M0+) RSA-2048 / 4096 Backend**: Constant-time CIOS Montgomery reduction engine with branchless conditional subtraction and sub-kilobyte stack allocation. Hardware verified on physical Target 2 (`nucleo-stm32c031c6` Cortex-M0+ @ 12 MHz) via Teleprobe (100% PASS on RSA-2048 public verification [2.38 s, 28.6M cyc with $e=65537$]).
- [x] ✅ **Target 2: ARMv6-M (Cortex-M0 / Cortex-M0+) Ed25519 Backend**: Complete extended twisted Edwards point arithmetic, point compression/decompression with RFC 8032 square-root recovery, and constant-time scalar multiplication. Hardware verified on physical Target 2 (`nucleo-stm32c031c6` Cortex-M0+ @ 12 MHz) via Teleprobe (100% PASS on basepoint compression [260 ms], basepoint decompression [755 ms], point addition [9.36 ms, 112.3k cyc], and 256-bit scalar multiplication [2.30 s, 27.6M cyc]).
