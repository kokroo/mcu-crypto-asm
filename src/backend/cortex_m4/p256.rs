...
pub fn mod_n_inv(out: &mut [u32; 8], in_: &[u32; 8]) {
    ...
    let mut state: [State; 2] = [
        State {
            fg: [
                ...
                FGInteger {
                    flip_sign: 0,
                    signed_value: [in_[0], in_[1], in_[2], in_[3], in_[4], in_[5], in_[6], in_[7], 0],
                },
            ],
            xy: [
                XYInteger { flip_sign: 0, value: [0; 8] },
                XYInteger { flip_sign: 0, value: [1 << 24, 0, 0, 0, 0, 0, 0, 0] },
            ],
        },
        State {
            fg: [
                FGInteger { flip_sign: 0, signed_value: [0; 9] },
                FGInteger { flip_sign: 0, signed_value: [0; 9] },
            ],
            xy: [
                XYInteger { flip_sign: 0, value: [0; 8] },
    ...
    for i in 0..24 {
        ...
        unsafe {
            P256_matrix_mul_fg_9(matrix[0], matrix[1], state[cur].fg.as_ptr(), &mut state[next].fg[0]);
            P256_matrix_mul_fg_9(matrix[2], matrix[3], state[cur].fg.as_ptr(), &mut state[next].fg[1]);
            P256_matrix_mul_mod_n(matrix[0], matrix[1], state[cur].xy.as_ptr(), &mut state[next].xy[0]);
            P256_matrix_mul_mod_n(matrix[2], matrix[3], state[cur].xy.as_ptr(), &mut state[next].xy[1]);
        }
    }

    let should_neg = ((state[0].xy[0].flip_sign ^ state[0].fg[0].flip_sign ^ (state[0].fg[0].signed_value[8] as i32)) & 1) as u32;
    unsafe {
...
pub fn scalarmult_fixed_base(out_mont_x: &mut [u32; 8], out_mont_y: &mut [u32; 8], scalar: &[u32; 8]) {
    ...
    for i in (0..32).rev() {
        {
            ...
            if i == 31 {
                ...
                unsafe {
                    P256_double_j(current_point.as_mut_ptr() as *mut u32, current_point.as_ptr() as *const u32);
                }
...
pub fn scalarmult_variable_base(
    ...
    unsafe {
        P256_double_j(table[7].as_mut_ptr() as *mut u32, table[0].as_ptr() as *const u32);
        for i in 1..8 {
            table[i] = table[7];
            P256_add_sub_j(
...
pub fn decode_public_key_mont(pk: &[u8], out_x_mont: &mut [u32; 8], out_y_mont: &mut [u32; 8]) -> Result<(), EcdhError> {
    let mut x = [0u32; 8];
    let mut y = [0u32; 8];
...
pub fn sign(
    ...
    if unsafe { P256_check_range_n(d.as_ptr()) } == 0 || unsafe { P256_check_range_n(k.as_ptr()) } == 0 {
...
pub fn verify(
    ...
    if unsafe { P256_check_range_n(r.as_ptr()) } == 0 || unsafe { P256_check_range_n(s.as_ptr()) } == 0 {
    ...
    unsafe {
        P256_double_j(pk_table[7].as_mut_ptr() as *mut u32, pk_table[0].as_ptr() as *const u32);
        for i in 1..8 {
