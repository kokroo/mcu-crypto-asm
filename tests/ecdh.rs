...
fn p256_public_key_encoding_matches_oracle() {
    ...
    // ECDH with generator must produce the public key's x-coordinate
    let generator_uncompressed: [u8; 65] = [
        0x04,
    ...
    let mut ss = [0u8; 32];
    p256::ecdh::shared_secret(&sk, &generator_uncompressed, &mut ss).unwrap();
    assert_eq!(ss, want[1..33], "ECDH(scalar, G).x must match public key x-coordinate");
}
